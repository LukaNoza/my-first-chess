...
function buildPiece(name, color, id, player) {
  ...
  return (
    "<div " + id + " " + player + ' class="game-piece">' + value + "</div>"
  );
}
...
function checkPawnRules(obj, x, y) {
  ...
  if (obj.x === x && collisionValue && collisionValue.color !== obj.color)
    result = false;
...
function checkKnightRules(initial, x, y) {
  ...
  if (
    (collisionValue && collisionValue.color !== ownColor) ||
    !collisionValue
  ) {
    if (
